...
def generate_sql(players, season_id=None, dry_run=False, restate_squads=True):
    ...
    lines.append('-- ====================================================================')
    lines.append('BEGIN;')
    lines.append('-- Ensure required extension, base tables, and fantasy schema exist (idempotent DDL)')
    ...
    lines.append("    END IF;")
    lines.append('')
    lines.append("    -- 4. Match or create players (strictly prevent duplicate creation if player exists anywhere)")
    lines.append("    INSERT INTO players (name, team_id, position, created_at, updated_at)")
    lines.append("    SELECT DISTINCT ON (LOWER(TRIM(t.name)), tm.id)")
    lines.append("           t.name, tm.id,")
    ...
    lines.append("    )")
    lines.append("    ORDER BY LOWER(TRIM(t.name)), tm.id, t.rank ASC;")
    ...
    lines.append("    WITH matched_players AS (")
    lines.append("        SELECT DISTINCT ON (t.rank)")
    lines.append("            t.rank,")
    lines.append("            t.final_price,")
    lines.append("            t.composite_index,")
    lines.append("            pl.id AS player_id")
    lines.append("        FROM tmp_player_pricing t")
    lines.append("        JOIN teams tm ON LOWER(TRIM(tm.name)) = LOWER(TRIM(t.team_name))")
    lines.append("                      OR UPPER(TRIM(COALESCE(tm.short_name, ''))) = UPPER(TRIM(t.team_code))")
    lines.append("        JOIN players pl ON LOWER(TRIM(pl.name)) = LOWER(TRIM(t.name))")
    lines.append("        ORDER BY t.rank, (pl.team_id = tm.id) DESC, pl.created_at ASC")
    lines.append("    )")
    lines.append("    INSERT INTO fantasy_player_prices (id, season_id, player_id, gameweek_id, base_price, rating, price, created_at)")
    lines.append("    SELECT DISTINCT ON (m.player_id)")
    lines.append("        gen_random_uuid(),")
    lines.append("        v_season_id,")
    lines.append("        m.player_id,")
    lines.append("        NULL, -- opening season price")
    lines.append("        m.final_price,")
    lines.append("        ROUND(GREATEST(LEAST(m.composite_index * 10, 10.0), 3.0), 2),")
    lines.append("        m.final_price,")
    lines.append("        NOW()")
    lines.append("    FROM matched_players m")
    lines.append("    ORDER BY m.player_id, m.rank ASC")
