...
func (r *PostgresStatsRepository) UpsertPlayerStat(ctx context.Context, stat *domain.PlayerStat) error {
	query := `
		...
		ON CONFLICT (player_id, competition_id, match_date) DO UPDATE SET
			...
			passing_attempts = player_stats.passing_attempts + EXCLUDED.passing_attempts,
			rushing_attempts = player_stats.rushing_attempts + EXCLUDED.rushing_attempts,
			completed_passes = player_stats.completed_passes + EXCLUDED.completed_passes,
			passing_tds = player_stats.passing_tds + EXCLUDED.passing_tds,
			rushing_tds = player_stats.rushing_tds + EXCLUDED.rushing_tds,
			interceptions_thrown = player_stats.interceptions_thrown + EXCLUDED.interceptions_thrown,
			receptions = player_stats.receptions + EXCLUDED.receptions,
			receiving_tds = player_stats.receiving_tds + EXCLUDED.receiving_tds,
			extra_points_tds = player_stats.extra_points_tds + EXCLUDED.extra_points_tds,
			drops = player_stats.drops + EXCLUDED.drops,
			flag_pulls = player_stats.flag_pulls + EXCLUDED.flag_pulls,
			pass_deflections = player_stats.pass_deflections + EXCLUDED.pass_deflections,
			interceptions = player_stats.interceptions + EXCLUDED.interceptions,
			defensive_tds = player_stats.defensive_tds + EXCLUDED.defensive_tds,
			safety = player_stats.safety + EXCLUDED.safety,
			qb_sacks = player_stats.qb_sacks + EXCLUDED.qb_sacks,
			def_sacks = player_stats.def_sacks + EXCLUDED.def_sacks,
			updated_at = NOW()
	`
