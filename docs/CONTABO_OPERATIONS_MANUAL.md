...
| Deploy status    | `~/scripts/deploy.sh`                          | Emails on **both** outcomes — success (with the deployed commit SHA, a link to it on GitHub, and the image's build time, read from OCI labels CI stamps on every image) and failure (with the health status and rollback outcome). |
| Alert delivery   | `~/scripts/alert.sh`                           | Sends via the Resend API; used by all scripts above.                                                                                                                                                 |
